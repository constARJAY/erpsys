$(document).ready(function() {
    ...
    // ----- DATATABLE -----
    $("#checkoutInvoiceTable").DataTable();
    function checkoutInvoiceTable(type, dateFrom, dateTo) {
        $("#reservationHistoryTable_parent").html(loadingAnimation);
        const data = {type, dateFrom, dateTo};

        if ($.fn.DataTable.isDataTable('#checkoutInvoiceTable')){
            $('#checkoutInvoiceTable').DataTable().destroy();
        }
        ...
        const getTypeData = function() {
            ...
            $.ajax({
                method:   "GET",
                url:      "checkout_invoice/getTypeData",
                async:    false,
                data,
                dataType: "json",
        ...
        html += `
            <table class="table table-hover table-striped table-bordered" id="checkoutInvoiceTable">
                <thead>
                    <tr>
        ...
        setTimeout(() => {
            ...
            $("#checkoutInvoiceTable").css({"min-width": "100%"}).removeAttr('width').DataTable({
                scrollX:        true,
                scrollCollapse: true,
                columnDefs: [
                    { targets: 0, width: 100 },
    ...
    $(".daterange").daterangepicker({
        ...
        type && checkoutInvoiceTable(type, dateFrom, dateTo);
    ...
    $(document).on("change", "#select_type", function() {
        ...
        checkoutInvoiceTable(type, dateFrom, dateTo);
    ...
    $(document).on("click", "#btnPrint", function() {
        ...
        $.ajax({
            method: "GET",
            url: "checkout_invoice/printAllCheckoutInvoice",
    ...
    $(document).on("click", "#btnExcel", function() {
        ...
        window.location.replace('checkout_invoice/downloadExcel?type='+type+'&dateFrom='+dateFrom+'&dateTo='+dateTo); 
    })
    ...
    $(document).on("click", ".btnView", function() {
        ...
        $.ajax({
            method: "POST",
            url: "checkout_invoice/getSpecificCheckoutInvoice",
            data: {clientID, type},
            dataType: "JSON",
            async: false,
            beforeSend: function() {
