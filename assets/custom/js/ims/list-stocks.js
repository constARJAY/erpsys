             ...
             success: function(data) {
                           ...
                           let assets =`
                            ...
                            <div class="card mt-2 bg-white rounded shadow-sm">
                             ...
                             let getStockInAssets     =  parseFloat(data["assets"][i].stockIN) || 0;
                             let getBorrowedAssets    = parseFloat(data["assets"][i].equipmentBorrowing) || 0;
                             let getReservedAssets      = parseFloat(data["assets"][i].reservedAsset) || 0;
                             let getDisposedAssets      = parseFloat(data["assets"][i].disposed) || 0;
                             let getReOrderAssets     = parseFloat(data["assets"][i].reOrderLevel) || 0;
                             let computeAvailableStocksAssets =0;
                             let computeTotalQuantityAssets   =0;
 
                             computeAvailableStocksAssets = getStockInAssets - getReOrderAssets   - getReservedAssets ;

                             computeAvailableStocksAssets = (computeAvailableStocksAssets <0 ? 0 : computeAvailableStocksAssets);
                             
                             if(computeAvailableStocksAssets <=0){
                                computeTotalQuantityAssets =0;
                             }else{
                                computeTotalQuantityAssets = computeAvailableStocksAssets + getReOrderAssets + getReservedAssets - (getBorrowedAssets !=0  ? getBorrowedAssets : 0)  ;
                                 ...
                                 <tr> 
                                     ...
                                     <td class="text-center">${formatAmount(computeAvailableStocksAssets)}</td>
                                     <td class="text-center">${formatAmount(computeTotalQuantityAssets)}</td>
                                 </tr>`;
                             }
                             assets +=`</tbody>
                             </table>`;
                            ...
                            for(var i=0; i<data["item"].length; i++){
                             ...
                             let getStockInItems     =  parseFloat(data["item"][i].stockIN) || 0;
                             let getStockOutItems    = parseFloat(data["item"][i].stockOut) || 0;
                             let getWithdrawnItems   = parseFloat(data["item"][i].materiaWithdrawalQuantity) || 0;
                             let getUnusedItems      = parseFloat(data["item"][i].Unused) || 0;
                             let getReservedItems    = parseFloat(data["item"][i].reservedItem) || 0;
                             let getComputeReservedItems    = parseFloat(data["item"][i].reserved) || 0;
                             let getReOrderItems     = parseFloat(data["item"][i].reOrderLevel) || 0;
                             let computeAvailableStocksItems =0;
                             let computeTotalQuantityItems   =0;
 
                             // computeAvailableStocksItems = getStockInItems - getReOrderItems - getWithdrawnItems  - getUnusedItems -  (getStockOutItems =0  ? getComputeReservedItems : getWithdrawnItems !=0 ? getStockOutItems : getStockOutItems < getComputeReservedItems ?   getComputeReservedItems - getStockOutItems  : getStockOutItems - getComputeReservedItems);
                             computeAvailableStocksItems = getStockInItems - getReOrderItems   - getReservedItems ;

                             computeAvailableStocksItems  = (computeAvailableStocksItems < 0 ? 0 : computeAvailableStocksItems);

                             if(computeAvailableStocksItems <=0){
                                computeTotalQuantityItems =0;
                             }else{
                                computeTotalQuantityItems = computeAvailableStocksItems + getReOrderItems + getReservedItems - (getStockOutItems !=0  ? getStockOutItems : 0) ;
                             ...
                             <tr> 
                                 ...
                                 <td class="text-center">${data["item"][i].reserved}</td> 
                                 <td class="text-center">${formatAmount(data["item"][i].reOrderLevel)}</td>
                                 <td class="text-center">${formatAmount(computeAvailableStocksItems)}</td>
                                 <td class="text-center">${formatAmount(computeTotalQuantityItems)}</td>
                            </tr>`;
                             }
                             item +=`</tbody>
                             </table>
