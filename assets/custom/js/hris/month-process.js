$(document).ready(function() {
    ...
    function headerTabContent(display = true) {
        if (display) {
            if (isImModuleApprover("hris_13month_tbl", "approversID") || allowedShow) {
    ...
    function forViewingContent() {
        ...
        let monthViewingData = getTableData(
            `hris_13month_tbl AS isrt 
            LEFT JOIN hris_employee_list_tbl AS helt USING(employeeID)`,
            `isrt.*, CONCAT(helt.employeeFirstname, ' ', helt.employeeLastname) AS fullname`,
            `isrt.employeeID != ${sessionID} AND monthStatus != 0 AND monthStatus != 4 AND monthStatus = 2`,
        ...
        <table class="table table-bordered table-striped table-hover" id="tableForViewing">
            ...
            html += `
            <tr class="${btnClass}" id="${encryptString(monthID)}" isForViewing="true">
                <td>${monthCode || "-"}</td>
                <td>${dateRange}</td>
                <td>${fullname}</td>
                <td>
