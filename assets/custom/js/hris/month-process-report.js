...
// ----- MODULE APPROVER -----
const moduleApprover = getModuleApprover("13th Month Report");
// ----- END MODULE APPROVER -----
...
function updateURL(view_id = 0, isAdd = false, isRevise = false) {
    if (view_id && !isAdd) {
        window.history.pushState("", "", `${base_url}hris/month_process_report?view_id=${view_id}`);
    } else if (isAdd) {
        if (view_id && isRevise) {
            window.history.pushState("", "", `${base_url}hris/month_process_report?add=${view_id}`);
        } else {
            window.history.pushState("", "", `${base_url}hris/month_process_report?add`);
        }
    } else {
        window.history.pushState("", "", `${base_url}hris/month_process_report`);
    }
}
// ----- END VIEW DOCUMENT -----
...
function myFormsContent() {
    ...
    let monthData = getTableData(
        ...
        LEFT JOIN hris_employee_list_tbl AS helt USING(employeeID)`,
        `isrt.*, CONCAT(helt.employeeFirstname, ' ', helt.employeeLastname) AS fullname`,
        `isrt.employeeID = ${sessionID}  AND monthStatus = 2`,
        `FIELD(monthStatus, 0, 1, 3, 2, 4, 5), COALESCE(isrt.submittedAt, isrt.createdAt)`
    );
...
// ---- GET ALL EMPLOYEE HAS PAYROLL IN RANGE OF DECLARED ----//
    ...
    $(document).on("click", "#btnExcel", function() {
        ...
        const url = `${base_url}hris/month_process_report/downloadExcel?monthID=${monthID}&dateFrom=${startDate}&dateTo=${endDate}`;
    ...
    <div class="row" id="form_inventory_receiving">
        ...
        <div class="col-sm-12">
            <div class="w-100">
                ...
                <div class="text-primary font-weight-bold mb-2" style="font-size: 1.5rem;">Details: 
                <button class="btn btn-info px-5 py-2 float-right" id="btnExcel"  monthID="${monthID}" startDate="${monthDateStart}"  endDate="${monthDateEnd}"> <i class="fas fa-file-excel"></i> Excel
                </button>
                </div>
