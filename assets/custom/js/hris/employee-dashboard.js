$(document).ready(function() {
    ...
    const allowedUpdate = isUpdateAllowed(88);
    if(!allowedUpdate){
...
// ----- MODULE APPROVER -----
const moduleApprover = getModuleApprover("Employee Dashboard");
// ----- END MODULE APPROVER -----
...
function updateURL(view_id = 0, isAdd = false, isRevise = false) {
    if (view_id && !isAdd) {
        window.history.pushState("", "", `${base_url}hris/employee_dashboard?view_id=${view_id}`);
    } else if (isAdd) {
        if (view_id && isRevise) {
            window.history.pushState("", "", `${base_url}hris/employee_dashboard?add=${view_id}`);
        } else {
            window.history.pushState("", "", `${base_url}hris/employee_dashboard?add`);
        }
    } else {
        window.history.pushState("", "", `${base_url}hris/employee_dashboard`);
    }
}
// ----- END VIEW DOCUMENT -----
...
function attendanceChart(startYear = moment().format("YYYY"),endYear = moment().format("YYYY")){
...
function myFormsContent() {
    ...
    const countPendingReport = getTableData(`hris_production_tbl`,
    `COUNT(productionStatus) AS pendingleaves`,
    `employeeID =${sessionID} AND productionStatus =1`);

    const countPendingLeave = getTableData(`hris_leave_request_tbl`,
    `COUNT(leaveRequestStatus) AS pendingleaves`,
    `employeeID =${sessionID} AND leaveRequestStatus  =1`);

    const countPendingOvertmine = getTableData(`hris_overtime_request_tbl`,
    `COUNT(overtimeRequestStatus) AS pendingovertime`,
    `employeeID =${sessionID} AND overtimeRequestStatus   =1`);

    const countTotalLeave = getTableData(`hris_employee_leave_tbl`,
    `SUM(leaveCredit) + SUM(leaveAccumulated) AS totalleave`,
    ...
    const getYearPeriod = getTableData(`hris_timekeeping_items_tbl`,
    ` DATE_FORMAT(scheduleDate, "%Y") as yearPeriod`,
    `employeeID = ${sessionID}  GROUP BY EXTRACT(year FROM scheduleDate)`);
...
<div class="row clearfix row-deck">
    <div class="col-xl-6 col-lg-12">
        <div class="card" style="box-shadow:none !important;">
            ...
            <div class="col-sm-12 col-md-4 col-lg-4"> 
                ...
                <select class="text-center btnYear select2" style="width:100%;">`;
    ...
    <div class="col-xl-3 col-lg-6">
        <div class="card">
            <div class="header bg-primary">
                <h2 class="font-weight-bold text-white">EVENT THIS WEEK</h2>
            </div>
    ...
    <div class="col-xl-3 col-lg-6">
        <div class="card">
            <div class="header bg-primary">
                <h2 class="font-weight-bold text-white">DEADLINES FOR THIS WEEK</h2>
            </div>
             <div class="card-body" style="color:black;height: 400px; overflow-y: auto;"> `;

             if(task.length > 0 || subTask.length > 0){
                task.map((task) =>{
