$(document).ready(function() {
    ...
    function getAllPayrollData(payrollID = 0) {
        ...
        if (payrollID) {
            $.ajax({
                method: "POST",
                url: "payroll_report/getAllPayrollData",
                data: { payrollID },
                dataType: "json",
                async: false,
	...
	function updateURL(view_id = 0, isAdd = false, isRevise = false) {
		if (view_id && !isAdd) {
			window.history.pushState("", "", `${base_url}hris/payroll_report?view_id=${view_id}`);
		} else if (isAdd) {
			if (view_id && isRevise) {
				window.history.pushState("", "", `${base_url}hris/payroll_report?add=${view_id}`);
			} else {
				// window.history.pushState("", "", `${base_url}hris/payroll_report?add`);
			}
		} else {
			window.history.pushState("", "", `${base_url}hris/payroll_report`);
		}
	}
	// ----- END VIEW DOCUMENT -----
    ...
    function getPayrollItems(payrollID = 0) {
        ...
        $.ajax({
            method: "POST",
            url: "payroll_report/getPayrollItems",
            data: { payrollID },
            dataType: "json",
            async: false,
            success: function(data) {
        ...
        <div class="row px-2">
			<div class="col-12 mb-2 text-right">
                <button class="btn btn-info"
                    id="btnPrint"
                    payrollID="${payrollID}"
                    payrollCode="${payrollCode}"><i class="fas fa-print"></i> PRINT</button>
            </div>
			${documentReviseNo}
	...
	$(document).on('click', `#btnPrint`, function() {
		...
		$.ajax({
            method: "POST",
			url: `payroll_report/printReport`,
			data: { payrollID, payrollCode },
			success: function (data) {
		...
		Swal.fire({
			...
			if (res.isConfirmed) {
				...
				setTimeout(() => {
					$.ajax({
						method:   "POST",
						url:      "payroll_report/createPayrollAdjustment",
						data:     { payrollID },
						async:    false,
						dataType: "json",
						success: function(data) {
	...
	$(document).on("click", "#btnReject", function () {
		...
		$("#modal_payroll_report_content").html(preloader);
		$("#modal_payroll_report .page-title").text("DENY PAYROLL");
		$("#modal_payroll_report").modal("show");
		let html = `
		...
		$("#modal_payroll_report_content").html(html);
	});
	...
	$(document).on("click", "#btnRejectConfirmation", function () {
		...
		const validate = validateForm("modal_payroll_report");
		if (validate) {
			let tableData = getTableData("hris_payroll_tbl", "", "payrollID = " + id);
			if (tableData) {
				let approversStatus = tableData[0].approversStatus;
	...
	function getConfirmation(method = "submit") {
		...
		$("#modal_payroll_report").text().length > 0 && $("#modal_payroll_report").modal("hide");
			...
			confirmation.then(res => {
				if (res.isConfirmed) {
					...
					setTimeout(() => {
						$.ajax({
							method:      "POST",
							url:         `payroll_report/savePayroll`,
							data,
							async:       false,
							dataType:    "json",
							beforeSend: function() {
					...
					if (res.dismiss === "cancel" && method != "submit") {
						if (method != "deny") {
							...
							$("#modal_payroll_report").text().length > 0 && $("#modal_payroll_report").modal("show");
						}
