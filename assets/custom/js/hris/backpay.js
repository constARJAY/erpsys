$(document).ready(function() {
    ...
    const allowedUpdate = isUpdateAllowed(95);
    if(!allowedUpdate){
...
// ----- MODULE APPROVER -----
const moduleApprover = getModuleApprover("back pay");
// ----- END MODULE APPROVER -----
...
function headerTabContent(display = true) {
    if (display) {
        if (isImModuleApprover("hris_back_pay_tbl", "approversID")) {
...
function forApprovalContent() {
    ...
    //     "isrt.*, CONCAT(employeeFirstname, ' ', employeeLastname) AS fullname,isrt.backPayID ,isrt.createdAt AS dateCreatedVoucher, pct.paymentRequestID , pct.createdAt AS dateCreatedPayment",
...
$(document).on("click", "#btnBack", function () {
    ...
    if (status != "false" && status != 0) {
        ...
        if (revise) {
            ...
            // data["reviseBackPayID"] = id;
            // delete data["backPayID"];
...
$(document).on("click", "#btnRejectConfirmation", function () {
    ...
    if (validate) {
        ...
        if (tableData) {
            ...
            // data["backPayID"] = $("[name=backPayID]").val()?.trim();
            // data["updatedBy"] = sessionID;
