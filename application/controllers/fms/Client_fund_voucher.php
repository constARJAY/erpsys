...
class Client_fund_voucher extends CI_Controller {
    ...
    {
        $data["title"] = "Client Fund Voucher";

        $this->load->view("template/header",$data);
        $this->load->view("fms/client_fund_voucher/index");
        $this->load->view("template/footer");
    }

    public function downloadExcel(){
        $clientFundRequestID = $this->input->get("id");
        // $this->clientFundRequestExcel($clientFundRequestID);
        if ($clientFundRequestID) {
            $clientFundRequestData = $this->operations->getTableData('fms_client_fund_request_tbl', '', "clientFundRequestID = '$clientFundRequestID' ", '', '', '');
            if ($clientFundRequestData) {
                $this->clientFundRequestExcel($clientFundRequestData[0]);
                // print_r($clientFundRequestData[0]);
            }
        }
    }

    public function clientFundRequestExcel($data) {
        $clientFundRequestID         = $data["clientFundRequestID"];
        if($data){

            str_replace(" ","","Hello world!");
            $code        = getFormCode("PCV", $data["createdAt"], $data["clientFundRequestID"]);
            ...
            $sheet->setCellValue('D1',"CLIENT FUND VOUCHER");
            $sheet->getStyle('D1:H1')->applyFromArray($clientFundVoucherHeader);
    
            $sheet->mergeCells('I1:K1');
            $sheet->setCellValue('I1',$code);
            $sheet->getStyle('I1:K1')->applyFromArray($clientFundVoucherCode);
            ...
            $sheet->getStyle('A4:H4')->applyFromArray($clientFundVoucherHeader);
            $sheet->getStyle('A4:H4')->applyFromArray($boldTextStyle);
    
            $sheet->mergeCells('I4:K4');
            $sheet->setCellValue('I4',"Amount");
            $sheet->getStyle('I4:K4')->applyFromArray($clientFundVoucherHeader);
            ...
            $clientFundRequestDetails = $this->operations->getTableData('fms_finance_request_details_tbl LEFT JOIN fms_liquidation_tbl USING(liquidationID)', '',
                                                                        "fms_liquidation_tbl.clientFundRequestID = '$clientFundRequestID' ", '', '', '');                                                  
            $limit      = count($clientFundRequestDetails);
            $totalAmountRequested = 0;
            for ($i = 0; $i < $limit; $i++) { 
                $sheet->mergeCells("A$rowNumber:H$rowNumber");
                $sheet->setCellValue("A$rowNumber",$clientFundRequestDetails[$i]["description"]." ( ".$clientFundRequestDetails[$i]["quantity"]."x )");
                $sheet->getStyle("A$rowNumber:H$rowNumber")->applyFromArray($normalTextStyle);
                $totalAmount = floatval($clientFundRequestDetails[$i]["quantity"]) * floatval($clientFundRequestDetails[$i]["amount"] ?? 0.00);
                ...
                $totalAmountRequested += floatval($clientFundRequestDetails[$i]["amount"]);
                $rowNumber++;
            }
            ...
            $sheet->mergeCells("A$rowNumber:E$rowNumber");
            $sheet->getStyle("A$rowNumber:E$rowNumber")->applyFromArray($clientFundVoucherHeader);
    
            $sheet->mergeCells("G$rowNumber:K$rowNumber");
            $sheet->getStyle("G$rowNumber:K$rowNumber")->applyFromArray($clientFundVoucherHeader);
